...
/*   Updated: 2025/10/30 10:42:53 by cbayousf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_path_tex(t_parse *parse)
{
	if (!parse->textures[NO].path || !parse->textures[NO].path[0])
		return (ft_putstr_fd("Error: NO texture path is NULL or empty\n", 2), 1);
	if (!parse->textures[SO].path || !parse->textures[SO].path[0])
		return (ft_putstr_fd("Error: SO texture path is NULL or empty\n", 2), 1);
	if (!parse->textures[WE].path || !parse->textures[WE].path[0])
		return (ft_putstr_fd("Error: WE texture path is NULL or empty\n", 2), 1);
	if (!parse->textures[EA].path || !parse->textures[EA].path[0])
		return (ft_putstr_fd("Error: EA texture path is NULL or empty\n", 2), 1);
	return (0);
}

int	load_texture(t_parse *parse)
{
	if (!parse || check_path_tex(parse))
		return (1);
